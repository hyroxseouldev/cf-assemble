    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="text-2xl font-bold text-[#2f2418] sm:text-3xl">{challenge.title}</h1>
                ...
                <p className="mt-2 inline-flex rounded-lg bg-[#f2e4ce] px-3 py-1 text-sm font-bold text-[#4a3724]">
                  {ddayLabel}
                </p>
              </div>
            </div>
            <Link
              href="/challenge-info"
              className="inline-flex h-11 items-center justify-center rounded-xl border border-[#84b19d] bg-[#e9f4ef] px-4 text-sm font-semibold text-[#2f5f4b] transition hover:bg-[#d9ece4]"
            >
              챌린지 핵심 정보 보기
            </Link>
          </div>
        ...
        <section className="rounded-3xl border border-[#e8cda1] bg-white/90 p-6 shadow-[0_10px_35px_-24px_rgba(80,45,9,0.55)] backdrop-blur sm:p-8">
          <p className="text-sm font-semibold tracking-[0.12em] text-[#8b6e4d]">DATE NAVIGATION</p>
          <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-[#ead7b6] bg-[#fff8ed] p-2 sm:px-3">
            <button
              type="button"
              onClick={() => setSelectedDateISO((prev) => shiftDate(prev, -1))}
              aria-label="이전 날짜"
              className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-[#d8c19b] bg-white text-2xl font-semibold leading-none text-[#5b4632] transition hover:bg-[#fdeac8]"
            >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex h-10 items-center justify-center rounded-lg border border-[#84b19d] bg-[#eaf7f1] px-4 text-sm font-semibold text-[#2f5f4b] transition hover:bg-[#dff2e9]"
            >
              오늘로 이동
            </button>
            <p className="text-sm text-[#6b543d]">{inPeriod ? "챌린지 기간 내 날짜" : "챌린지 기간 외 날짜"}</p>
          </div>
        </section>

        <section className="rounded-3xl border border-[#e8cda1] bg-white/90 p-6 shadow-[0_10px_35px_-24px_rgba(80,45,9,0.55)] backdrop-blur sm:p-8">
          <p className="text-sm font-semibold tracking-[0.12em] text-[#8b6e4d]">DAILY QUOTE</p>
          {selectedDateQuote ? (
            <blockquote className="mt-3 rounded-2xl border border-[#edd9b5] bg-[#fff7e9] px-4 py-4">
              <p className="text-lg font-medium leading-relaxed text-[#3a2c1e]">
                &ldquo;{selectedDateQuote.quote}&rdquo;
              </p>
              <footer className="mt-2 text-sm font-semibold text-[#7b6147]">- {selectedDateQuote.author}</footer>
            </blockquote>
          ) : (
            <p className="mt-3 text-sm text-[#6b543d]">선택한 날짜에는 등록된 다이어트 명언이 없습니다.</p>
          )}
        </section>

        <section className="rounded-3xl border border-[#e8cda1] bg-white/90 p-6 shadow-[0_10px_35px_-24px_rgba(80,45,9,0.55)] backdrop-blur sm:p-8">
          <p className="text-sm font-semibold tracking-[0.12em] text-[#8b6e4d]">DAILY WORKOUT</p>
          {mission && missionView ? (
            <>
              <h2 className="mt-3 text-3xl font-bold text-[#2f2418]">{mission.title}</h2>
              ...
              {mission.youtubeUrl ? (
                <a
                  ...
                  className="mt-5 inline-flex items-center rounded-xl bg-[#84b19d] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#5f987f]"
                >
                  운동 영상 보기
                </a>
              ) : null}
              {embedUrl ? (
                <div className="mt-5 overflow-hidden rounded-2xl border border-[#ebd8b7] bg-[#fff8ed]">
            ...
            <div className="mt-4 rounded-2xl border border-[#ebd8b7] bg-[#fff8ed] px-4 py-4 text-[#5b4834]">
